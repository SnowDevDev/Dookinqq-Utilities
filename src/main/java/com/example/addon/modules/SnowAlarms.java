...
public class SnowAlarms extends Module {
    ...
    public SnowAlarms() {
        super(AddonTemplate.CATEGORY, "Dookinqq Alarms", "Plays an alarm sounds when a player joins or is now in render distance.");
    }
    ...
    private void playSound() {
        if (mc.player != null) {
            Vec3d pos = Compat.getPos(mc.player);
            ...
            mc.world.playSound(null, pos.x, pos.y, pos.z, sound, mc.player.getSoundCategory(), volumeSetting, pitchSetting);
        }
    }
            ...
            if (packet.getActions().contains(PlayerListS2CPacket.Action.ADD_PLAYER)) {
                if (useListJ.get()) {
                    for (PlayerListS2CPacket.Entry entry : packet.getPlayerAdditionEntries()) {
                        String playerName = Compat.getProfileName(entry.profile());
